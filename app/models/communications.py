from sqlalchemy import Column, Integer, Text, ForeignKey, DateTime, String, JSON, Boolean
from sqlalchemy.sql import func
from app.core.database import Base


class BusinessChatbot(Base):
    __tablename__ = "business_chatbots"

    id = Column(Integer, primary_key=True)
    business_id = Column(Integer, ForeignKey("businesses.id", ondelete="CASCADE"), nullable=False)
    name = Column(String(150), nullable=False)
    system_prompt = Column(Text, nullable=True)
    is_store_bot = Column(Boolean, default=False, nullable=False)
...
class BusinessDocument(Base):
    ...
    id = Column(Integer, primary_key=True)
    business_id = Column(Integer, ForeignKey("businesses.id", ondelete="CASCADE"), nullable=False)
    chatbot_id = Column(Integer, ForeignKey("business_chatbots.id", ondelete="CASCADE"), nullable=True)
...
class CommunicationSession(Base):
    ...
    id = Column(Integer, primary_key=True)
    business_id = Column(Integer, ForeignKey("businesses.id", ondelete="CASCADE"), nullable=False)
    chatbot_id = Column(Integer, ForeignKey("business_chatbots.id", ondelete="SET NULL"), nullable=True)
